...
class BabyLayingHensActivity : AppCompatActivity() {
    private val selectedItems: ArrayList<String?> = arrayListOf(null, null, null, null, null)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_baby_laying_hens)
...
//                arrayListOf(2, 3, 4) -> chosenId = 9
            ...
            if (countNotNullElement >=4) {
                if (chosenId != 0) {
                    val detailBabyLayingHens = Intent(this, DetailBabyLayingHensActivity::class.java).apply {
                        putExtra("CHOSENID", chosenId)
                    }
                    startActivity(detailBabyLayingHens)
                }
            } else {
