...
class AdultLayingHensActivity : AppCompatActivity() {
    private val selectedItems: ArrayList<String?> = arrayListOf(null, null, null, null, null)

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_adult_laying_hens)
        ...
        buttonSend.setOnClickListener { view ->
            ...
            if (countNotNullElement >=3) {
                if (chosenId != 0) {
                    val detailAdultLayingHens = Intent(this, DetailAdultLayingHensActivity::class.java).apply {
                        putExtra("CHOSENID", chosenId)
                    }
                    startActivity(detailAdultLayingHens)
                }
            } else {
